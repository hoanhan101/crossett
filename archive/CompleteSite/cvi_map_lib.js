/**
 * cvi_map_lib.js 2.0 (09-May-2008)
...
var cvi_map = {
	...
	add: function(image, options) {
		...
		if(image.tagName.toUpperCase() == "IMG" && map) {
			var defopts = { "radius" : cvi_map.defaultRadius, "opacity" : cvi_map.defaultOpacity, "bordercolor" : cvi_map.defaultBordercolor, "areacolor" : cvi_map.defaultAreacolor, "noborder" : cvi_map.defaultNoborder, "nofade" : cvi_map.defaultNofade, "showcoords" : cvi_map.defaultShowcoords, "imgsrc" : cvi_map.defaultImgsrc, "mapid" : cvi_map.defaultMapid };
			if(options) {
	...
	modify: function(image, options) {
		try {		
			...
			if(canvas && bgrnd && map) {
				...
				if(isVM||isIE) {image.movefunc = image.onmousemove; }else {image.movefunc = image.getAttribute("onmousemove"); }
				if(mapid!='') {
					var tmp = document.getElementById(mapid);
					if(tmp) {map.innerHTML = tmp.innerHTML;}
				}else {map.innerHTML = image.areas; }
				for(var j=0;j<map.areas.length;j++) {
					if(map.areas[j].shape.match(/(rect|poly|circle)/i)) {
						if(window.opera||map.areas[j].coords!='') {
							...
							if(isVM||isIE) {
								func = map.areas[j].onmouseover; map.areas[j].onmouseover = new Function('setAreaOver(this,"'+canvas.id+'","'+bcolor+'","'+acolor+'",'+opacity+','+noborder+','+nofade+','+image.jg+');'+func); 
								func = map.areas[j].onmouseout; map.areas[j].onmouseout = new Function('setAreaOut(this,"'+canvas.id+'",'+nofade+','+image.jg+');'+func); 
							}else {
						...
						for(var j=0;j<map.areas.length;j++) {
							if(map.areas[j].shape.match(/(rect|poly|circle)/i)) { 
								if(window.opera||map.areas[j].coords!='') {
									atr = map.areas[j].id;
									if(isVM||isIE) {
										func = map.areas[j].onmousemove; map.areas[j].onmousemove = new Function('getCoords(event,"'+image.mapname+'","'+atr+'","'+image.id+'",'+t.x+','+t.y+','+iw+','+ih+','+pw+','+ph+');'+func); 
									}else {
